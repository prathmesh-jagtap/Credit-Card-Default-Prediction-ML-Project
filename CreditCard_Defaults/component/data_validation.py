...
from CreditCard_Defaults.entity.config_entity import DataValidationConfig
from CreditCard_Defaults.entity.artifact_entity import DataIngestionArtifact, DataValidationArtifact
...
class DataValidation:
    ...
    def validate_dataset_schema(self) -> bool:
        try:
            logging.info("Dataset Schema Validation")
            train_df, test_df = self.get_train_and_test_df()

            # Cleaning and validating data
            schema_file = self.data_validation_config.schema_file_path
            schema = read_yaml_file(schema_file)  # reading schema file
            columns = schema.get("columns")
            cat_col = schema.get("domain_value")  # cat columns

            for df in (train_df, test_df):
                if df.shape[1] == len(columns) and list(columns.keys()) == list(df.columns):
