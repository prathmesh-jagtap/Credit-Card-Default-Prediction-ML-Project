...
def evaluate_classification_model(model_list: list, X_train: np.ndarray, y_train: np.ndarray,
    ...
    try:
        ...
        for model in model_list:
            ...
            # score on training and testing dataset
            ...
            model_accuracy = (2 * (train_as * test_as)) / (train_as + test_as)
            diff_test_train_acc = abs(test_as - train_as)

            # logging all important metric
            logging.info(f"{'>>'*30} Score {'<<'*30}")
            logging.info(
                f"Train f1-Score \t\t Test f1-Score \t\t Average Score")
            logging.info(f"{train_fs} \t\t {test_fs} \t\t {model_accuracy}")

            logging.info(f"{'>>'*30} Loss {'<<'*30}")
            logging.info(
                f"Diff test train accuracy score: [{diff_test_train_acc}].")
            logging.info(
                f"Train accuracy based on accuracy score: [{train_as}].")
            logging.info(
                f"Test accuracy based on accuracy score: [{test_as}].")
            logging.info("\n\n")

            # if model accuracy is greater than base accuracy and train and test score is within certain thershold
            # we will accept that model as accepted model
            if train_fs >= base_accuracy and test_fs >= 0.48 and model_accuracy > base_accuracy:
                base_accuracy = model_accuracy
                metric_info_artifact = MetricInfoArtifact(model_name=model_name,
                                                          model_object=model,
                                                          train_accuracy_score=train_as,
                                                          test_accuracy_score=test_as,
                                                          train_f1score=train_fs,
                                                          test_f1score=test_fs,
...
class ModelFactory:
    ...
    def update_property_of_class(instance_ref: object, property_data: dict):
        try:
            ...
            logging.info(f"{'+'*15}property_data : {property_data} {'+'*15}")
    ...
    def execute_grid_search_operation(self, initialized_model: InitializedModelDetail, input_feature,
        ...
        Args:
            initialized_model (InitializedModelDetail): estimator: Model object, 
            param_grid: dictionary of paramter to perform search operation
            input_feature (array): your all input features
            output_feature (array): Target/Dependent features

        Returns:
            GridSearchedBestModel: Function will return GridSearchOperation object
        """
        try:
